...
const examModes = [
  { id: 'english-bangla', label: 'English to Bangla', hint: 'Pick the Bangla meaning.' },
  { id: 'bangla-english', label: 'Bangla to English', hint: 'Pick the English word.' },
  { id: 'english-synonym', label: 'English to Synonym', hint: 'Pick the synonym.' },
  { id: 'english-antonym', label: 'English to Antonym', hint: 'Pick the antonym.' },
  { id: 'english-meaning', label: 'English to Definition', hint: 'Pick the definition.' },
  { id: 'meaning-english', label: 'Definition to English', hint: 'Pick the word.' },
  { id: 'synonym-english', label: 'Synonym to English', hint: 'Pick the matching word.' },
  { id: 'antonym-english', label: 'Antonym to English', hint: 'Pick the matching word.' },
];
...
export const AllWords = () => {
  ...
  const accuracy = examQuestionsAsked.length ? correctCount / examQuestionsAsked.length : 0;
  ...
  const fetchWords = useCallback(async ({
    ...
    try {
      ...
      console.error(err);
      showToast('Could not load vocabulary vault', 'error');
    } finally {
      setLoading(false);
    }
  }, [showToast]);
  ...
  const handleAnswer = (answer) => {
    ...
    if (isAuthenticated) {
      api.recordTestAnswer({ wordId: examQuestion.wordId, isCorrect }).catch(() => {});
    }
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 pt-8 pb-16 space-y-8 animate-fade-in">
          ...
          <div className="lg:col-span-7 p-6 md:p-8 space-y-5">
            ...
            <div className="space-y-3">
              ...
              <p className="max-w-2xl text-sm leading-relaxed text-zinc-500">
                Search the full vault, collect today&apos;s target words, revise them with focused tests, and grow your vocabulary one clean session at a time.
              </p>
            </div>
          ...
          <div className="lg:col-span-5 bg-zinc-950 text-white p-6 md:p-8 flex flex-col justify-between gap-8">
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-2xl bg-white/10 p-4">
                ...
                <p className="text-xs font-bold uppercase tracking-wider text-zinc-400">Vault Words</p>
              </div>
              <div className="rounded-2xl bg-white/10 p-4">
                <Brain className="h-5 w-5 text-emerald-200" />
                <p className="mt-4 text-2xl font-extrabold">4</p>
                <p className="text-xs font-bold uppercase tracking-wider text-zinc-400">Test Modes</p>
              </div>
            </div>
            <div className="space-y-3">
              <p className="text-xs font-bold uppercase tracking-wider text-zinc-400">Today&apos;s rhythm</p>
              <div className="flex flex-wrap gap-2">
                {['Search', 'Collect', 'Revise', 'Learned'].map((step) => (
                  <span key={step} className="rounded-lg bg-white/10 px-3 py-1.5 text-xs font-bold text-zinc-100">
                    {step}
                  </span>
                ))}
              </div>
      ...
      <section className="space-y-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div className="space-y-1">
            <span className="text-xs font-bold text-indigo-500 uppercase tracking-widest">Full Vault Exam</span>
            <h2 className="text-2xl font-extrabold tracking-tight text-zinc-900 font-display">
              Test Yourself
            </h2>
            <p className="text-sm text-zinc-500">
              Competitive questions from the whole word storage. Correct answers add 1 point, wrong answers subtract 1.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {examModes.map((mode) => (
            ...
            >
              ...
              <span className="mt-1 block text-xs text-zinc-500">{mode.hint}</span>
            </button>
          ))}
        </div>
            ...
            {examLoading ? (
              ...
              <>
                <div className="rounded-3xl bg-zinc-50 p-8 text-center space-y-3">
                  ...
                  {examQuestion.word && (
                    <Link to={`/words/${examQuestion.wordId}`} className="text-xs font-bold text-zinc-400 hover:text-indigo-600">
                      View source word
                    </Link>
                  )}
                </div>
                  ...
                  <div className="rounded-3xl bg-zinc-50 p-6 space-y-4">
                    <div className="space-y-1 text-center">
                      ...
                      <p className="text-sm text-zinc-600">
                        Final score <span className="font-bold">{score}</span> / {EXAM_SESSION_SIZE}{" "}
                        · Accuracy <span className="font-bold">{correctCount}</span>/{answeredCount}{" "}
                    ...
                    {missedWords.length > 0 ? (
                      <div className="space-y-2">
                        ...
                        <div className="flex flex-wrap gap-2 justify-center">
                          {Array.from(new Set(missedWords)).slice(0, 8).map((wordId) => (
                            <Link
                              key={`${wordId}-review`}
                              to={`/words/${wordId}`}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="space-y-1">
          <span className="text-xs font-bold text-indigo-500 uppercase tracking-widest">Vocabulary Vault</span>
          <h1 className="text-3xl font-extrabold tracking-tight text-zinc-900 font-display">
            Vault Dictionary ({totalWords})
          </h1>
          <p className="text-xs text-zinc-400">All saved and seeded English vocabulary.</p>
        </div>
      ...
      <section className="p-4 rounded-2xl border border-zinc-200 bg-white shadow-premium space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {/* Sort By Dropdown */}
          <div className="space-y-1">
            <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider block">Sort By</span>
            <select
              value={sortBy}
          ...
          {/* Order Dropdown */}
          <div className="space-y-1">
            <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider block">Order</span>
            <select
              value={order}
      ...
      <section>
        {loading ? (
          ...
          <div className="text-center py-20 border border-dashed border-zinc-200 rounded-3xl bg-zinc-50/50 space-y-4">
            ...
            <div className="space-y-1">
              <h3 className="text-base font-bold text-zinc-800">No words match search</h3>
              <p className="text-xs text-zinc-400">Try a different keyword or clear the search field.</p>
            </div>
          </div>
        ) : viewMode === 'grid' ? (
